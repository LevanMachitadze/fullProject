...
const Invoices = ({ isLight, filterStatus, setArrayLength }) => {
  ...
  useEffect(() => {
    setArrayLength(invoices.length);
  }, [invoices, setArrayLength]);
  ...
  const filteredInvoices = filterStatus
    ? invoices.filter(
        (invoice) => invoice.status.toLowerCase() === filterStatus
      )
    : invoices;
    ...
    >
      {loading ? (
        ...
        <div>
          ...
          <div
            className={`p-6 border rounded-lg shadow-md ${
              isLight ? 'bg-white text-[#0C0E16]' : 'bg-[#1E2139] text-white'
            } mb-16`}
          >
              ...
              <div className='mt-8 bg-gray-50 rounded-lg'>
                ...
                <div
                  className={`flex justify-between items-center mt-4 ${
                    isLight
                      ? 'bg-[#373B53] text-white'
                      : 'bg-[#141625] text-white'
                  } w-full h-[80px] rounded-b-[8px] p-4`}
                >
        ...
        <div className='space-y-4'>
          {filteredInvoices.map((invoice, index) => (
            <div
              key={index}
              className={`p-4 border rounded-lg shadow-md flex items-center justify-between cursor-pointer ${
                isLight ? 'bg-white text-[#0C0E16]' : 'bg-[#1E2139] text-white'
              }`}
              onClick={() => handleInvoiceClick(invoice)}
            >
              <div className='font-bold text-[#0C0E16]'>#{invoice.id}</div>
              <div className='text-gray-500 text-sm'>
                Due {invoice.date} {invoice.month} {invoice.year}
              </div>
              <div className='font-bold text-lg text-[#0C0E16]'>
      ...
      {deleteAlert && (
        <div className='fixed inset-0 bg-black bg-opacity-50 z-10 flex justify-center items-center'>
          <div
            className={`bg-white p-6 rounded-lg shadow-lg z-20 w-[400px] ${
              isLight ? 'bg-white' : 'bg-[#1E2139]'
            }`}
          >
